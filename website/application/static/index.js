...
socket.on("connect", async function () {
  ...
  var fileform = $("form#fileForm").on("submit", async function (e) {
    e.preventDefault()

    var formData = new FormData(uploadForm);

    let file_input = document.getElementById("file");
    let user_input = file_input.value;
    let user_name = await load_name();
    ...
    xhr.send(formData,user_name)
  
    socket.emit("event", {
      message: '<p>' + user_input + ' <a href="/download/' + user_input + '">Download</a></p>',
      name: user_name,
    });
  
    xhr.addEventListener('load', function() {
      filemessages.innerHTML += '<p>' + xhr.responseText + '</p>';
      fileInput.value = '';
  });
  });
});
